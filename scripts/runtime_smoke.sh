...
USER_AGENT="${RUNTIME_SMOKE_USER_AGENT:-SF_SlimeHUD-CI/2.0.3 (https://github.com/wickidcow/SF_SlimeHUD)}"
...
cp "$ADDON_JAR" "$WORK_DIR/plugins/SF_SlimeHUD2.0.3.jar"
...
if ! grep -Fq 'Enabling SlimeHUD v2.0.3' "$CONSOLE_LOG"; then
    echo "${FAMILY} ${MC_VERSION}: SlimeHUD 2.0.3 enable line was not observed." >&2
    cat "$CONSOLE_LOG" >&2 || true
    exit 1
fi
if [[ -n "$WIT_VERSION_ID" ]]; then
